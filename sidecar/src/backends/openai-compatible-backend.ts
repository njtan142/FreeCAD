...
export class OpenAICompatibleBackend implements AgentBackend {
  ...
  readonly description = 'OpenAI-compatible providers (Ollama, LM Studio, Groq)';
  ...
  async initialize(config: BackendConfig): Promise<void> {
    this.config = {
      baseUrl: config.baseUrl || process.env.OPENAI_COMPATIBLE_BASE_URL || 'http://localhost:11434/v1',
      apiKey: config.apiKey || process.env.OPENAI_COMPATIBLE_API_KEY || 'no-key',
      model: config.model || process.env.OPENAI_COMPATIBLE_MODEL || 'llama3.2',
      temperature: config.temperature ?? (process.env.OPENAI_COMPATIBLE_TEMPERATURE ? parseFloat(process.env.OPENAI_COMPATIBLE_TEMPERATURE) : 0.7),
      maxTokens: config.maxTokens ?? (process.env.OPENAI_COMPATIBLE_MAX_TOKENS ? parseInt(process.env.OPENAI_COMPATIBLE_MAX_TOKENS, 10) : 4096),
      ...config,
    };

    this.sessionId = this.generateSessionId();
    console.log(`[OpenAICompatibleBackend] Initialized session: ${this.sessionId}`);
    console.log(`[OpenAICompatibleBackend] Base URL: ${this.config.baseUrl}`);
    console.log(`[OpenAICompatibleBackend] Model: ${this.config.model}`);
    console.log(`[OpenAICompatibleBackend] API Key: ${this.config.apiKey === 'no-key' ? '(local/no-key)' : '(set)'}`);
  }

  private generateSessionId(): string {
    return `openai-compat-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
  }
  ...
  async sendMessage(
    ...
    try {
      ...
      const model = openai(this.config.model || 'llama3.2', {
      ...
      const result = streamText({
        ...
        onToolCall: async ({ toolCall }: { toolCall: { id: string; name: string; args: Record<string, any> } }) => {
          ...
          console.log(`[OpenAICompatibleBackend] Executing tool: ${name}`, JSON.stringify(args).substring(0, 200));
          try {
            const toolResult = await this.executeViaBridge(name, args);
            return { toolCallId: id, result: toolResult };
          } catch (error) {
            console.error(`[OpenAICompatibleBackend] Tool execution failed for ${name}:`, error);
      ...
      for await (const delta of result.fullStream) {
        if (delta.type === 'text-delta') {
          ...
          console.log(`[OpenAICompatibleBackend] Tool result for ${delta.toolName}:`, JSON.stringify(delta.result).substring(0, 200));
          toolResults.set(delta.toolCallId, delta.result);
        } else if (delta.type === 'finish') {
          console.log(`[OpenAICompatibleBackend] Stream finished. Total content: ${fullContent.length} chars, Tool calls: ${toolCalls.length}`);
        } else if (delta.type === 'error') {
          console.error(`[OpenAICompatibleBackend] Stream error:`, delta.error);
        }
      }
      ...
      console.error('[OpenAICompatibleBackend] Error:', error);
      return {
        content: '',
...
Objects: ${context.documentInfo.objectCount}`;
  ...
  private async initializeMCPTools(tools: MCPTool[]): Promise<any[]> {
    ...
    console.log(`[OpenAICompatibleBackend] Initialized ${mcpTools.length} MCP tools`);
    return mcpTools;
  }
...
try:
    ...
    print(json.dumps({'success': False, 'error': 'This tool requires MCP connection'}))`;
      ...
      case 'list_chat_sessions':
        return `print(json.dumps({'error': 'Tool ${toolName} is not supported via the OpenAI-compatible backend. Use the Claude or OpenCode backend for session management operations.'}))`;

      default:
        return `print(json.dumps({'error': 'Tool ${toolName} is not yet supported via the OpenAI-compatible backend. Use execute_freecad_python for this operation or use the Claude/OpenCode backend.'}))`;
    }
  }
...
try:
  ...
  async disconnect(): Promise<void> {
    this.sessionId = '';
    console.log('[OpenAICompatibleBackend] Disconnected');
  }
}
