...
try:
    ...
    print(json.dumps({'success': False, 'error': 'This tool requires MCP connection'}))`;
      ...
      case 'list_chat_sessions':
        return `print(json.dumps({'error': 'Tool ${toolName} is not supported via the Vercel AI backend. Use the Claude or OpenCode backend for session management operations.'}))`;

      default:
        return `print(json.dumps({'error': 'Tool ${toolName} is not yet supported via the Vercel AI backend. Use execute_freecad_python for this operation or use the Claude/OpenCode backend.'}))`;
    }
  }
