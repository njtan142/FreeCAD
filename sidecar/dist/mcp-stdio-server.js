...
async function main() {
    ...
    server.tool('execute_freecad_python', `Execute Python code in FreeCAD's environment. Use this to create, modify, or query CAD models.
The code runs in FreeCAD's Python interpreter with access to FreeCAD, Part, Draft, Arch, and all other modules.
Always call doc.recompute() after creating or modifying objects.`, { code: zod_1.z.string().describe('Python code to execute in FreeCAD') }, async ({ code }) => {
        try {
            const result = await executePython(code);
            return {
                content: [{ type: 'text', text: result.output || result.error || 'Code executed (no output)' }],
            };
        }
        catch (error) {
            return {
    ...
    // Query model state
    server.tool('query_model_state', 'Get comprehensive information about the current FreeCAD document including all objects, their types, properties, and relationships.', {}, async () => {
        ...
        try {
            ...
            return {
                content: [{ type: 'text', text: parsed.success ? JSON.stringify(parsed.data, null, 2) : `Error: ${parsed.error}` }],
            };
        }
        catch (error) {
            return {
    ...
    // List objects
    server.tool('list_objects', 'List all objects in the active FreeCAD document with their names, labels, types, and visibility.', {}, async () => {
    ...
    server.tool('get_object_properties', 'Get all properties of a specific object in the FreeCAD document.', { objectName: zod_1.z.string().describe('Name of the object to inspect') }, async ({ objectName }) => {
    ...
    // Get selection
    server.tool('get_selection', 'Get the currently selected objects in the FreeCAD viewport.', {}, async () => {
    ...
    // Redo
    server.tool('redo', 'Redo the last undone operation in FreeCAD.', {}, async () => {
        try {
            const result = await executePython(`import FreeCAD as App; doc = App.ActiveDocument; doc.redo(); doc.recompute(); print('Redo done')`);
            return { content: [{ type: 'text', text: result.output || 'Redo performed' }] };
        }
        catch (error) {
            return { content: [{ type: 'text', text: `Error: ${error instanceof Error ? error.message : String(error)}` }] };
        }
    ...
    // Save document
    server.tool('save_document', 'Save the current FreeCAD document.', { filePath: zod_1.z.string().optional().describe('Optional file path for Save As') }, async ({ filePath }) => {
        const code = filePath
        ...
        try {
            const result = await executePython(code);
            return { content: [{ type: 'text', text: result.output || 'Document saved' }] };
        }
        catch (error) {
            return { content: [{ type: 'text', text: `Error: ${error instanceof Error ? error.message : String(error)}` }] };
        }
